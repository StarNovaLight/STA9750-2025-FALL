...
```{r}
total_countries<-COUNTRY_TOP_10|>
    summarize(name=unique(country_name))|>
    format_titles() |>
    datatable(style='bootstrap5',options=list(searching=FALSE, info=FALSE)) 
...
```
Netflix is a streaming platform, that helps viewers enjoy other countries movies, with the provided subtitles and connects people. For instance the Non-English film that spent the most weeks in the Global top 10 on Netflix is the [`r popular_Non_English$show_title`]{style='color:cyan;'}, as it spent consecutive [`r popular_Non_English$most_weeks`]{style='color:cyan;'} in the charts.
...
Netflix longest running film is [`r longest_run$show_title`]{style='color:cyan;'}. It's approximately about s [`r longest_run$longest_film`]{style='color:cyan;'}  minutes long, providing almost 4 hours of entertainment.
...
The most recent film to reach number 1 after their debut week was Unknown Number: The High School Catfish. It appears once on the list when looking at the most recent weeks of when these shows reached #1 as took 2 weeks. There is a total of [`r shows_total`]{style='color:cyan'} films that didn’t debut at number 1 but eventually reached the top spot.
...
Netflix have a variety of shows and films that comes from different cultures. I took the opportunity to go research some Hindi language films. In the list above are some of the most popular films in India, some of the most popular films are Animal and Haseen Dillruba. For instance Animal has a total of over [`r first_show$total_viewers`]{style='color:cyan;'}  viewers when on the top 10 and a total of [`r first_show$total_weekly_views`]{style='color:cyan;'} weekly views.  Haseen Dillruba has a total of [`r second_show$total_weekly_hours`]{style='color:cyan;'} weekly hours watched. These films were completely in Hindi, but don't worry if you want to watch films that are in a different language subtitles are very useful. These films entertained me and were a joy to experience.
