...
Netflix has a lot of influence all over the world, as currently I found that are a total of [`r num_countries$country_name`]{style='color:cyan;'}  countries. This show that Netflix has the impacted and influence world's entertainment streaming services.
...
```
Netflix is a streaming platform, that help viewers enjoy other countries movies/shows, with the provided subtitles and connects people. For instance the Non-English film that spent the most weeks in the Global top 10 on Netflix is the [`r popular_Non_English$show_title`]{style='color:cyan;'}, as it spent consecutive [`r popular_Non_English$most_weeks`]{style='color:cyan;'} in the charts.
...
Netflix longest running film is [`r longest_run$show_title`]{style='color:cyan;'}. It's approximately about s [`r longest_run$longest_film`]{style='color:cyan;'} minutes long, which means 48 hours. This show is providing 2 full days’ worth of entertainment.
...
Number_1<-COUNTRY_TOP_10|>
  ...
  filter(country_name=='United States',(weekly_rank!=1)==(cumulative_weeks_in_top_10==1),year(week)==2025)|>
...
The most recent film to reach number 1 that has never been there before was Unknown Number: The High School Catfish. It appears once on the list when looking at the most recent weeks of when these shows reached #1 as took 2 weeks. However, repeated shows/films up in the data as sometimes they would reclaim the number #1 spot after being dethrone. However there is a total of [`r shows_total`]{style='color:cyan'} shows/films that didn’t debut at number 1.
...
COUNTRY_TOP_10|>
  ...
  filter(cumulative_weeks_in_top_10==1)|>
  summarize(total_countries=sum(n_distinct(country_name)))|>
  ungroup()|>
...
In the list above shows the number of shows/films and their respective season that made it on the top 10 Netflix list in their debut week.
...
India_Media<- COUNTRY_TOP_10|>
  group_by(show_title)|>
  filter(country_name=='India',country_name!='United States')|>
  summarize(most_popular=max(cumulative_weeks_in_top_10))|>
  slice_max(most_popular, n=5)|>
  format_titles() |>
  datatable(style='bootstrap5',options=list(searching=FALSE, info=FALSE)) 
...
second_show<-GLOBAL_TOP_10|>
  group_by(show_title)|>
  filter(show_title=='Khakee: The Bihar Chapter')|>
...
Netflix have a variety of shows and films that comes from different cultures. I took the opportunity to go watch some Hindi language films. I was the most popular Hindi language films such as The Railway Men- The Untold Story of Bhopal 1984 and Khakee: The Bihar Chapter. for instance The Railway Men- The Untold Story of Bhopal 1984 has a total of over [`r first_show$total_viewers`]{style='color:cyan;'}  viewers when on the top 10 and a total of [`r first_show$total_weekly_views`]{style='color:cyan;'} weekly views. Khakee: The Bhar Chapter has [`r second_show$total_weekly_hours`]{style='color:cyan;'} hours watched.  These films were completely in Hindi, but don't worry if you want to watch films that are in a different language subtitles are very useful. These films entertained me and brought me joy to watch.
